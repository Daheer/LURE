...
Set the path to the unzipped folder in line 7 of [transform.py](transform.py), then run code below to resize the images to 224 x 224
```bash
python transform.py
```
...
```bash
#--input_image -> should point to a folder (not file) containing images you want to infer on
#--output_file -> should point to a json file to store results to be used in the next step (generate_IDK)

python output_LURE.py --mode 'inference' --cfg-path /path/to/config.yaml --gpu-id gpu-id  --input_image /path/to/image_folder --output_file /path/to/output.jsonl 
```
...
```bash
#--input -> path to the intermediary json you specified during inference mode
#--output -> should point to a json file to store intermediary results to be used in the next step (rewrite)
python generate_IDK.py --input /path/to/caption_file.jsonl --output /path/to/idk_caption_file.json
```
...
```bash
#--input_image -> should point to a folder (not file) containing images you want to infer on
#--input_caption -> path to the json where results from previous step (generate_IDK) were saved
#--output_file -> should point to a json file to store final results 
python output_LURE.py --mode 'rewrite' --cfg-path /path/to/config.yaml --gpu-id gpu-id --input_caption /path/to/idk_caption_file  --input_image /path/to/image_file --output_file /path/to/output.jsonl 
```
### Other
